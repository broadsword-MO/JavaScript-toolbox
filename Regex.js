...
let alphabetRegex = /[a-z]/gi; // or simpler /[A-z]/g
...
// To create a negated character set, you place a caret character "^" after the opening bracket and before the one or more characters you do not want to match. Like this [^thingsThatWillNotBeMatched]. Any other position and the caret will be treated as another negated character
...
// ================== \w ===================
// Regex shorthand "\w" is equal to "[A-Za-z0-9_]" or simpler "[A-z0-9_]", all letters, numbers and underscore
let shortHand = /\w/g;
...
// REVIEW Double check this solution
let username = 'Z97';
...
//                   camelCase
// ===================================================
// ================== camelCase a variable name separated by space(s) ===================
function camelize(str) {
    return (
        str
            // REVIEW Most answers show this. Why all the extra regex?
            // .replace(/(?:^\w|[A-Z]|\b\w)/g, (letter, index) =>
            //     index === 0 ? letter.toLowerCase() : letter.toUpperCase()
            // )
            // Simpler
            .replace(/\b\w/g, (letter, index) =>
                index === 0 ? letter.toLowerCase() : letter.toUpperCase()
            )
            .replace(/\s+/g, '')
    );
...
console.log(camelize('first variable name')); // firstVariableName
console.log(camelize('FirstVariable Name')); // firstVariableName
console.log(camelize('First variableName')); // firstVariableName
console.log(camelize('FirstVariableName')); // firstVariableName
...
function toCamelCase(filename) {
    ...
    parts.length > 1 && (extension = parts.pop().toLowerCase());
    let name = parts
        .join(' ')
        .replace(/[-_.]|%20/g, ' ')
...
console.log(toCamelCase('my -file_.name.Js')); // myFileName.js
console.log(toCamelCase('This_ isYour%20other-file.html')); // thisIsYourOtherFile.html

// ================== Sanitize filename of unneeded characters ===================
// Mine. Replaces: everything except letters, numbers and these [#&(),'.-], with a space
function sanitize(filename) {
    let extension;
    // Separate name/filename at any period(s) into array items
    const parts = filename.split('.');
    // If array is more than one item, pop off the last and retain as the extension
    parts.length > 1 && (extension = parts.pop().toLowerCase());
    let name = parts
    // Take the rest and put back together with any period(s)
        .join('.')
        // Replace one or more of _, %20, or anything other than alphanumerics and these [#&(),'.-], with a space
        .replace(/(_|%20|[^\w#&(),'.-])+/g, ' ')
        // Add a space between alphanumerics and a Capital or L Parentheses
        .replace(/([a-z0-9])([A-Z]|\()/g, '$1 $2');
        // If there is an extension, put it back on the end of the filename with a period, otherwise just return the fixed name
    return extension ? `${name}.${extension}` : name;
}

console.log(
    '🔥sanitize >>',
    sanitize("This _is 2.1.3Your%20%20-file's.html"),
    typeof sanitize("This _is 2.1.3Your%20%20-file's.html")
); // This is 2.1.3 Your -file's.html string
console.log(
    '⚡sanitize >>',
    sanitize('This _10.9^isYour%20%20 & other-brother*.file(12-12-12).html')
); // This 10.9 is Your & other-brother .file (12-12-12).html

// ================== Match everything but ___ ===================
// Mine. Replaces all letters with '' except those in the .replace() function
function sanity() {
    let string = 'qwertyuAiopasCdfghjklzCxcDvbnm';
    return string.replace(/[A-z]/g, (match) =>
        /[ABC]/.test(match) ? match : ''
    );
}

console.log('🔥sanity >>', sanity(), typeof sanity()); // ACC string
